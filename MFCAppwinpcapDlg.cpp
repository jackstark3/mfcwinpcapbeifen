...
DWORD WINAPI sinff_CapThread(LPVOID lpParameter);

DWORD __stdcall sniff_CapThread(LPVOID lpParameter)
{
	return 0;
}
...
DWORD WINAPI sniff_CapThread(LPVOID lpParameter);         /*可能在后边定义*/
...
CMFCAppwinpcapDlg::CMFCAppwinpcapDlg(CWnd* pParent /*=nullptr*/)
	: CDialogEx(IDD_MFCAPPWINPCAP_DIALOG, pParent)
{
	m_hIcon = AfxGetApp()->LoadIcon(IDR_MAINFRAME);
}

void CMFCAppwinpcapDlg::DoDataExchange(CDataExchange* pDX)
{
	CDialogEx::DoDataExchange(pDX);
	...
	DDX_Control(pDX, IDC_EDIT8, m_editNIpv6);
	DDX_Control(pDX, IDC_EDIT6, m_editNArp);
	DDX_Control(pDX, IDC_EDIT7, m_editNHttp);
	DDX_Control(pDX, IDC_EDIT4, m_editNOther);
...
BEGIN_MESSAGE_MAP(CMFCAppwinpcapDlg, CDialogEx)
	ON_WM_SYSCOMMAND()
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
	ON_EN_CHANGE(IDC_EDIT8, &CMFCAppwinpcapDlg::OnEnChangeEdit8)
	ON_BN_CLICKED(IDC_BUTTON1, &CMFCAppwinpcapDlg::OnBnClickedButton1)
	ON_BN_CLICKED(IDC_BUTTON2, &CMFCAppwinpcapDlg::OnBnClickedButton2)
	ON_NOTIFY(LVN_ITEMCHANGED, IDC_LIST1, &CMFCAppwinpcapDlg::OnLvnItemchangedList1)
	ON_NOTIFY(NM_CUSTOMDRAW, IDC_LIST1, &CMFCAppwinpcapDlg::OnNMCustomdrawList1)
	ON_BN_CLICKED(IDC_BUTTON5, &CMFCAppwinpcapDlg::OnBnClickedButton5)
	ON_BN_CLICKED(IDC_BUTTON4, &CMFCAppwinpcapDlg::OnBnClickedButton4)
END_MESSAGE_MAP()


// CMFCAppwinpcapDlg 消息处理程序

BOOL CMFCAppwinpcapDlg::OnInitDialog()
{
...
void CMFCAppwinpcapDlg::OnSysCommand(UINT nID, LPARAM lParam)
{
...
void CMFCAppwinpcapDlg::OnPaint()
{
	if (IsIconic())
	{
...
//显示。
HCURSOR CMFCAppwinpcapDlg::OnQueryDragIcon()
{
...
//开始按钮
void CMFCAppwinpcapDlg::OnBnClickedButton1()                        /*可能bug*/
{
	...
	memset(&(this->npacket), 0, sizeof(pktcount*));                                                /*改动过，结果待定*/
	this->sniff_updateNPacket();

	if (this->sniff_startCap() < 0)
		return;
...
//结束按钮
void CMFCAppwinpcapDlg::OnBnClickedButton2()
{
	// TODO: 在此添加控件通知处理程序代码
...
//列表
void CMFCAppwinpcapDlg::OnLvnItemchangedList1(NMHDR* pNMHDR, LRESULT* pResult)
{
	...
	this->sniff_updateEdit(index);
	this->sniff_updateTree(index);
	*pResult = 0;
}


//保存按钮
void CMFCAppwinpcapDlg::OnBnClickedButton5()
{
	// TODO: 在此添加控件通知处理程序代码
	if (this->sniff_saveFile() < 0)
		return;
}

//读取按钮
void CMFCAppwinpcapDlg::OnBnClickedButton4()
{
	// TODO: 在此添加控件通知处理程序代码
	//读取之前将ListCtrl清空
	this->m_listCtrl.DeleteAllItems();
	...
	this->m_localDataList.RemoveAll();				//每次一开始就将以前存的数据清空掉
	this->m_netDataList.RemoveAll();
	memset(&(this->npacket), 0, sizeof(pktcount*));//各类包计数清空                                                 /*改动过，结果未定*/
	...
	{
		int ret = this->sniff_readFile(FileDlg.GetPathName());
		if (ret < 0)
			return;
	}
}
...
//改变ListCtrl每行颜色
void CMFCAppwinpcapDlg::OnNMCustomdrawList1(NMHDR* pNMHDR, LRESULT* pResult)
{
	...
	*pResult = 0;
	...
	if (CDDS_PREPAINT == pNMCD->nmcd.dwDrawStage)
	{
		*pResult = CDRF_NOTIFYITEMDRAW;
		...
		datapkt* local_data = (datapkt*)this->m_localDataList.GetAt(pos);
...
//初始化winpcap
int CMFCAppwinpcapDlg::sniff_initCap()
{
	devCount = 0;
...
//开始捕获
int CMFCAppwinpcapDlg::sniff_startCap()
{
...
m_ThreadHandle = CreateThread(NULL, 0, sinff_CapThread, this, 0, threadCap);
...
DWORD WINAPI sinff_CapThread(LPVOID lpParameter)
{
	...
	CMFCAppwinpcapDlg* pthis = (CMFCAppwinpcapDlg*)lpParameter;
	...
	while ((res = pcap_next_ex(pthis->adhandle, &header, &pkt_data)) >= 0)
	{
		...
		//分析出错或所接收数据包不在处理范围内
		if (analyze_frame(pkt_data, data, &(pthis->npacket)) < 0)                    /*后后面解决*/
			continue;
		...
		if (0x0806 == data->ethh->type)
		{
			...
			struct  in_addr in;
			in.S_un.S_addr = (u_long)data->iph->saddr;                            /*强制转换ulong*/
		...
		if (0x0806 == data->ethh->type)
		{
			...
			struct  in_addr in;
			in.S_un.S_addr = (u_long)data->iph->daddr;											/*强制转换ulong*/
			buf = CString(inet_ntoa(in));
		}
		else if (0x86dd == data->ethh->type) {
			int n;
...
//更新信息
int CMFCAppwinpcapDlg::sniff_updateEdit(int index)
{
	POSITION localpos, netpos;
	localpos = this->m_localDataList.FindIndex(index);
	netpos = this->m_netDataList.FindIndex(index);

	datapkt* local_data = (datapkt*)(this->m_localDataList.GetAt(localpos));
...
//更新统计数据
int CMFCAppwinpcapDlg::sniff_updateNPacket()
{
	CString str_num;
...
//更新树形控件
int CMFCAppwinpcapDlg::sniff_updateTree(int index)
{
	POSITION localpos;
	CString str;
	int i;
	...
	if (0x0806 == local_data->ethh->type)							//ARP
	{
		...
		str.Format(_T("源IP："));
		struct in_addr in;
		in.S_un.S_addr = (u_long)local_data->iph->saddr;								/*强制转换ulong*/
		str.AppendFormat(CString(inet_ntoa(in)));
		this->m_treeCtrl.InsertItem(str, ip);

		str.Format(_T("目的IP："));
		in.S_un.S_addr = (u_long)local_data->iph->daddr;
		str.AppendFormat(CString(inet_ntoa(in)));										/*强制转换ulong*/
		this->m_treeCtrl.InsertItem(str, ip);
...
int CMFCAppwinpcapDlg::sniff_saveFile()
{
	CFileFind find;
...
int CMFCAppwinpcapDlg::sniff_readFile(CString path)
{
	int res, nItem, i;
	struct tm* ltime;
	CString timestr, buf, srcMac, destMac;
	...
	CMFCAppwinpcapDlg* pthis = this;						//些代码改造自lixsinff_CapThread，为节约工作量，故保留pthis指针
	pcap_t* fp;
